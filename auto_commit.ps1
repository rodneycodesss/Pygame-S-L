...
Copy-Item "snakes and ladders.py" "snakes_and_ladders_backup.py" -Force

# Remove the working file if it exists
Remove-Item "snakes and ladders.py" -ErrorAction SilentlyContinue

# Define code blocks (each as a string)
$blocks = @(
@""""""
...
# For each block, append to the file, stage, commit, and push
foreach ($block in $blocks) {
    Add-Content "snakes and ladders.py" $block
    git add "snakes and ladders.py"
    # Use the first comment or docstring line as the commit message
    $firstLine = ($block -split "`n")[0].Trim()
    if ($firstLine -eq '') { $firstLine = 'Add code block' }
